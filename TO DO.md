...
- [x] Make the main title slowly pulse from 0.98 to 1.02 in size
  ...
  - [x] Refactor the main view so that instead of geometry readers dividing screen height and width, we give everything normal font numbers. Run base iphone 17 simulations testing the height/width numbers, and compare screenshots before and after
    - [x] Replace MainMenuView's title and menu-button Text font sizes (currently `maxScreenDimension`-derived fractions) with fixed point sizes, keeping existing frame/padding values as-is
    - [x] Replace MainMenuView's top-bar icon frame sizes (profile/trophy/settings/tutorial, currently `maxScreenDimension / 15` etc.) with fixed point sizes
