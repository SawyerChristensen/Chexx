...
- [x] Create an extensive CPU testing suite to make sure the CPU works as intended. Be able to log exact time differences and % increases in efficiency.
  ...
  - [ ] Redo the depth 1-5 old-vs-new CPU benchmark with 3 runs per depth, averaged, for more precise before/after numbers than the current single-run-per-depth results
