...
- [~] Add AI components to CPU? (TensorFlow, PyTorch) (AlphaZero loop on GPU?) (ideas)
  ...
  - [~] If the above concludes it's worth pursuing, design and build an offline self-play data-generation pipeline (many CPU-vs-CPU games at high search depth) to produce training positions/labels
    ...
    - [ ] Run the pipeline to generate a small sample dataset (e.g. 20-50 games) as a smoke test, verify the output, and document the format/location for future model-training work
