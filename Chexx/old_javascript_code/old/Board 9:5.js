...
const Board = () => {
  ...
  // Generate initial board layout and state object
  const addHexagon = (key, color, piece) => {
    initialBoardState[key] = piece;
    hexagons.push(
      //<TouchableOpacity key={key} onPress={() => handleHexagonClick(key)}>
        <Hexagon x={currentX} y={currentY} radius={radius} color={color} piece={piece} />
      //</TouchableOpacity>
    );
  };

  addHexagon("f6", hexColors[0], null);
  // ... call addHexagon for other hexagons ... *********************************


  // Create board state from initial state object
  const [boardState, setBoardState] = useState(initialBoardState);

  const handleHexagonClick = (key) => {
    if (selectedPiece) {
      // Move piece to new hexagon
      const newBoardState = { ...boardState, [key]: selectedPiece.type, [selectedPiece.source]: null };
      setBoardState(newBoardState);
      setSelectedPiece(null);
    } else if (boardState[key]) {
      // Select piece from current hexagon
      setSelectedPiece({ type: boardState[key], source: key });
    }
  };
